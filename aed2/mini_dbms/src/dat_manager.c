...
long writeRecord_dat( FILE *datFile, void *record, size_t recordSize ){
    ...
    if ( fwrite(record, recordSize, 1, datFile) != 1 ) { fprintf(stderr, "Erro ao escrever um arquivo"); }

    return newRecordOffset;
}

void *readRecord_dat( FILE *datFile, long offset, size_t recordSize ){
    if( fseek(datFile, offset, SEEK_SET) ) { fprintf(stderr, "Erro em 'readRecordFile()'"); };
...
size_t updateRecord_dat( FILE *datFile, long offset, void *record, size_t recordSize ){
    if ( fseek(datFile, offset, SEEK_SET) ){ fprintf(stderr, "Erro em updateRecordFile()"); }

    size_t writeReturn = fwrite(record, recordSize, 1, datFile);
    if ( writeReturn != 1 ) { fprintf(stderr, "Erro ao atualizar um arquivo"); }
